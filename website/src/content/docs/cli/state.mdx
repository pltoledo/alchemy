...
<resource>.json`, plus one `output.json` per stage holding the stack's
...
alchemy state list my-app/prod           # resource files under my-app/prod
...
Print persisted state as JSON. A file path prints that record; a
...
```sh
# one resource record
alchemy state read my-app/prod/Bucket.json
...
`output.json` cannot be deleted independently. Alchemy removes it with its
stage.
